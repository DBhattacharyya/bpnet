C
C Released version in March 2020, version 2.1.15
C
C
...
c     1 energy(1,i),yangle(1,i),' BASE(1,i)',base(1,i)
	...
	if(oligo.eq.1.and.(base(2,i).eq.(i-1).and.base(2,i-1).eq.i)) then
	   ierror=1
	   return
	endif  
!         write(*,331) i,prd(i)
...
160   format('#====='/'# Generated by BPFIND release MarcH 2021, version
     1 2.1.15'/'#=====')
...
c     1 bpair(i)
c	   if(ires4.gt.0) then
             write(81,162) ires1(i),base1(i),ires2(i),ires3,ires4
c	   elseif(ires3.gt.0) then
c             write(81,162) ires1(i),base1(i),ires2(i),ires3
c	   elseif(ires2(i).gt.0) then
c             write(81,162) ires1(i),base1(i),ires2(i)
c	   else
c             write(81,162) ires1(i), base1(i)
c	   endif
           ENDDO
         K=1
         ENDIF
...
c       write(*,*) 'NdbHlx',(ndoub(jl,1),ndoub(jl,2),jl=1,NdbHlx)
	   ...
	   if(nohlx.eq.0) then
           write(9,17) ires1(j),ires2(j),bpair(j)(1:4),bpair(j)(5:5),
     1  base1(j),base2(j)
	   endif
...
C May 9, 2018
         ...
         enddo
         if(ndoub(kl+1,1)-ndoub(kl,2).le.5) then
...
17	format(i5,1x,i5,a4,1x,a1,1x,2a1)
